...
export default function ChatifyPage() {
  ...
  const musicSidebarRef = useRef(null);
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      console.log('Connected to socket server');
      // Request rooms for this user only
      socket.emit('get_my_rooms', userId);
    });
  ...
  const confirmUsername = () => {
    ...
    socket.emit(action, { 
      ...
      if (response.success) {
        ...
        setChat([]);
        setShowUsernameModal(false);
        // Refresh my rooms list after creating
        socket.emit('get_my_rooms', userId);
      } else {
        setError(response.error);
      }
  ...
  const confirmDeleteRoom = () => {
    ...
    socket.emit('delete_room', { 
      ...
      if (response.success) {
        setMyRooms((prev) => prev.filter((r) => r.name !== roomToDelete));
        if (room === roomToDelete) handleExitRoom();
        setError('');
        setShowDeleteModal(false);
        setDeletePassword('');
  ...
  return (
    <div className="chatify-wrapper">
      ...
      <aside className={`sidebar ${showChatSidebar ? 'open' : ''}`}>
        ...
        <nav>
          <ul>
            <li className="active">My Private Rooms</li>
            <li style={{ fontWeight: 'bold' }}>My Room List</li>
            {myRooms.map((roomData, i) => (
              <li key={i} className="room-list-item">
                <span onClick={() => setRoom(roomData.name)}>{roomData.name}</span>
                <button onClick={() => handleDeleteRoom(roomData.name)}>✕</button>
              </li>
            ))}
          </ul>
        </nav>
      ...
      <aside className={`songs-sidebar ${showMusicSidebar ? 'open' : ''}`} ref={musicSidebarRef}>
        <h3>Songs 🎶</h3>
        <div className="search-container">
          <input
            ref={searchInputRef}
      ...
      {/* Footer Section - Only shown when not in a room */}
      {!joined && (
        <footer className="chatify-footer">
          <div className="footer-content">
            <span>🔥☕ Made with passion and caffeine by Lakhan Sharan</span>
